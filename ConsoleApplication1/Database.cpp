...
{	
    ...
    {
		printf("Database::RawMysqlQueryCall - SQL Error: '%s'.", mysql_error(m_pMYSQL));
		printf("Database::RawMysqlQueryCall - Query: '%s'.", strQuery.c_str());
        return false;
    }
...
void Database::EscapeString(std::string& str)
{
	...
	mysql_real_escape_string(m_pMYSQL, strResult, str.c_str(), (unsigned long)str.size());
